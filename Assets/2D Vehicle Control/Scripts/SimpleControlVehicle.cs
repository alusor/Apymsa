...
public class SimpleControlVehicle : MonoBehaviour {
    ...
    private void CheckUpgrades()
    {
        ...
        {
            float BobinaUpgrade = PlayerPrefs.GetInt(AutoActual + "Bobina") * 0.1f;
            maxSpeed = gears[actualGear].GearMaxSpeed + (1 * BobinaUpgrade);
        }
        if (PlayerPrefs.HasKey(AutoActual + "Bomba"))
        {
            float BombaUpgrade = PlayerPrefs.GetInt(AutoActual + "Bomba") * 0.1f;
            acceleration = gears[actualGear].GearAcceleration + (1 * BombaUpgrade);
        }
        if (PlayerPrefs.HasKey(AutoActual + "Cables"))
        {
            float CablesUpdate = PlayerPrefs.GetInt(AutoActual + "Cables") * 0.1f;
            maxSpeed = gears[actualGear].GearMaxSpeed + (1 * CablesUpdate);
        }
        if (PlayerPrefs.HasKey(AutoActual + "Inyector"))
        {
            float InyectorUpdate = PlayerPrefs.GetInt(AutoActual + "Inyector") * 0.1f;
            acceleration = gears[actualGear].GearAcceleration + (1 * InyectorUpdate);
        }
        if (PlayerPrefs.HasKey(AutoActual + "Turbo"))
        {
            float TurboUpdate = PlayerPrefs.GetInt(AutoActual + "Turbo") * 0.1f;
            //Insertar aqui variable de Turbo + (1 * TurboUpdate);
	...
	public void ShiftGears() {
        ...
        if (nextGear && StartCarrer.startcarrer == 1)
        {
        ...
        else if (backGear && StartCarrer.startcarrer == 1)
        {
			// Verify if the array have more gears.
			if(actualGear > 0){
