...
public class DashGUI : MonoBehaviour {
	...
	public Image speedArrow; // Used to rotate the arrow.
	...
	*/
    public 	Text speed; // Text with the actual speed of the vehicle.
	public Text kmText; // Text with the text Km/H
	public Text gear; // Text with the atual gear.
	...
	void Start () {
        vehicleControl = FindObjectOfType<SimpleControlVehicle>();
		if(!vehicleControl){
	...
	void Update () {
		...
		Quaternion rotation = Quaternion.AngleAxis(
			(vehicleControl.GetComponent<Rigidbody2D>().velocity.x / vehicleControl.maxSpeed) * -165,
			Vector3.forward);
		if (rotation.z < 0)
